...
export class TimerService {
  ...
  pausedTime: number = 0;
  savedLengths: number[] = [];
  startAnimation: Subject<number> = new Subject<number>();
  stopAnimation: Subject<number> = new Subject<number>();
  ...
  startTimerTracking() {
    ...
    if (this.currentMode === 'pomodoroFinished' && this.breakLength > 0) {
      ...
      this.startAnimation.next(this.savedLengths[1]);
      ...
      this.startAnimation.next(this.savedLengths[0]);
